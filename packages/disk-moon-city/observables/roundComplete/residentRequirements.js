...
import { normalizePercentage } from '../../utils/number.js';
import { fillTextEnd } from '../../utils/string.js';
import { DISTRIBUTION_TYPE } from '../../utils/city.js';
...
/**
 * Verarbeitet die Energieanforderungen der Einwohner.
 * @param {import('../../classes/Player.js').default} player
 */
function energy(player, status) {
  return of(player.city).pipe(
    map(city => {
      const lines = [];
      const groups = [
 ...
 * @param {import('../../classes/Player.js').default} player
 */
function cityRecruitResidents(player, status) {
  return of(player.city).pipe(
    map(city => {
      const lines = [];
      const groups = [
      ...
      if (city.resident.recruiting) {
        ...
        var value = city.resident.getIncomingRecruits();
      ...
      const difference = normalizePercentage(
        1 -
          city.getMaxStorageValue(STORAGE_TYPE.HUMAN, value) /
            (city.getMaxStorageValue(STORAGE_TYPE.HUMAN, value) + value)
      );
