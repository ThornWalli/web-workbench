...
const data = computed(() => {
    ...
    .reduce<{
      ...
      (result, log) => {
        ...
        result.needed.values.push(
          log.player.city.costValue[STORAGE_TYPE.MINERAL_ORE] || 0
        );
        result.production.values.push(
          log.player.city.storageValue[STORAGE_TYPE.MINERAL_ORE] || 0
        );
        result.storage.values.push(
          log.player.city.maxProductionValue[STORAGE_TYPE.MINERAL_ORE] || 0
        );
        return result;
      },
      {
  ...
  needed.values.push(
    core.currentPlayer.city.getCostValue(RESOURCE_TYPE.MINERAL_ORE)
  );
