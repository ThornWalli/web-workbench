...
export default ({ core }) => {
  ...
  function setMenuItems() {
    ...
    options.edit.disabled =
      ...
      selectedItems.find(fsItem => fsItem.locked || fsItem instanceof Trashcan);
    ...
    options.webLinkEdit.disabled =
      selectedItems.length < 1 ||
      selectedItems.find(item => !item.fsItem.meta.get(ITEM_META.WEB_URL));
    ...
    options.emptyTrashcan.disabled =
      selectedItems.length < 1 ||
      selectedItems.find(item => !(item.fsItem instanceof Trashcan));
          ...
          action() {
            ...
            return Promise.all(
              selectedItems
                ...
                .map(async ({ fsItem }) => {
                  return Promise.all(
                    Array.from((await fsItem.getItems()).values()).map(
                      async fsItem => {
                        // return fsItem.remove({ recursive: true });
                        const path = await fsItem.getPath();
                        return core.executeCommand(
                          `remove "${path}" -r -ignore`
                        );
                      }
                    )
                  );
    ...
    selectedItems
      ...
      .forEach(selectedItem => {
        ...
        [
          ...
          model[String(name)] = fsItem.meta.has(name)
