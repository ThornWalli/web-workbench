import PaletteColor from '@web-workbench/disk-extras13/webPaint/lib/classes/PaletteColor';
...
export default new Palette({
  locked: true,
  name: 'Windows 256 Color',
  description:
    'A palette inspired by the classic Windows 256 color set, including VGA colors, web-safe colors, and grayscale.',
  colors: generateWindows98_256ColorPalette().map(
    color => new PaletteColor({ color })
  )
});
